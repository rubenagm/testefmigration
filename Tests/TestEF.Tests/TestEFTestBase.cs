...
namespace TestEF.Tests
{
    public abstract class TestEFTestBase : AbpIntegratedTestBase<TestEFTestModule>
    {
        ...
        protected void UsingDbContext(Action<TestEFDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TestEFDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TestEFDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TestEFDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TestEFDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TestEFDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TestEFDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TestEFDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TestEFDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TestEFDbContext, Task<T>> func)
        {
            T result;
